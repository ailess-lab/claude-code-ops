...
| repo-baseline | 打磨中 | 仓库基线体检：核对 description/topics/README/LICENSE/Discussions，缺啥补啥。**补对外发布门面检查**（可见性/脱敏/链接/文案一致，发布前置 0 步，6/13 事故后补） |
| competitive-intel | 打磨中 | 竞品情报采集与深度分析：支持浅层（元数据+Issues）和深层（源码+项目结构），产物写入 intel/ |
| promo-plan | 打磨中 | 推广计划制定：产品事实校准 → 竞品调研 → 平台调研 → **多方向发散** → 每方向写样稿 → 评价优选 → 完整文案 → QA → 风险预案 → 排期，产物写入 content/ 和 intel/。**补步骤10发布后追踪与倒查**（0 数据强制倒查链接/平台/账号/文案，6/13 教训） |
| reddit-ops | 打磨中 | Reddit 操作（opencli 内置命令）：浏览帖子、发评论、查用户状态。养号和社区互动用。**补账号初始化前置**（新号默认头像/0 历史直接发被 spam filter 拦，6/13 教训） |
